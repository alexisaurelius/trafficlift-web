...
export default async function CroDashboardPage() {
  ...
  const [audits, auditsOrdered, auditsCompleted] = await Promise.all([
    prisma.audit.findMany({
      where: { userId: user.id, targetKeyword: CRO_AUDIT_KEYWORD },
    ...
    prisma.audit.count({ where: { userId: user.id, targetKeyword: CRO_AUDIT_KEYWORD } }),
    prisma.audit.count({ where: { userId: user.id, targetKeyword: CRO_AUDIT_KEYWORD, status: "COMPLETED" } }),
  ]);
